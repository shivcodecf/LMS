...
const SearchPage = () => {
  ...
  const sending = data?.courses;

  console.log("sending", sending);

  console.log("selected", selectedCategories);

  const isEmpty = !data?.courses || data?.courses?.length === 0;
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 mt-5 ">
      ...
      <div className="flex flex-col md:flex-row gap-10">
        <Filter handleFilterChange={handleFilterChange} data={data} />
        <div className="flex-1">
          {isLoading ? (
            data?.courses?.map((_, idx) => <CourseSkeleton key={idx} />)
          ) : isEmpty ? (
            <CourseNotFound />
          ) : (
